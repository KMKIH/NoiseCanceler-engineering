...
| 씬 전환 후 GameManager가 이전 FocusManager를 참조하는 문제 | Additive 로드 중 두 씬이 공존할 때 범위 없는 전역 탐색 수행 | Inspector 참조 또는 현재 씬으로 제한한 탐색 사용 | [Additive 씬 전환 중 이전 씬의 FocusManager를 참조한 문제](docs/trouble-shooting/focus-manager-scene-transition-reference-problem.md) |
