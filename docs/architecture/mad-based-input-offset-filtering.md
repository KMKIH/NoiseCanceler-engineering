...
리듬게임에서는 사용자의 입력 시점과 실제 판정 시점 사이의 차이를 입력 오프셋으로 보정한다.

입력 오프셋은 적은 수의 입력 데이터를 기반으로 계산하기 때문에, 한두 번의 오입력만으로도 계산 결과가 크게 달라질 수 있다. 따라서 이러한 값이 최종 오프셋에 미치는 영향을 줄이기 위해 MAD 기반 이상치 제거를 적용한다.

## 설계 목표

- 적은 수의 입력에서도 흔들리지 않는 보정값 계산
- 고정 범위가 아닌 데이터 분포 기반 이상치 판별
- 정상 입력 보존과 극단값 제거의 균형
